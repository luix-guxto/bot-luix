  ...
  try {
    ...
    switch (command) {
	 ...
	 if(!isGroup) return reply(mess.only.group)
      ...
      case 'link':
        if (!isGroup) return reply(mess.only.group)
		if(!isGroupAdmins) reply(mess.only.admin)
        ...
        case 'chances':
          ...
          case 'chamce':
            case 'serás':
              ...
              teks = `*Nome do bot* : ${client.user.name}\n*Dono* : *Luix_Guxto*\n*Numero do dono* : \nwa.me/553199949012\n*@ do bot* : @${client.user.jid.split('@')[0]}\n*Prefixo* : " ${prefix} "\n*Total de contatos bloqueados* : ${blocked.length}\n*O bot esta ativo a* :\n${kyun(uptime)}`
              buffer = await getBuffer(client.user.imgUrl)
              client.sendMessage(from, buffer, image, {
                caption: teks, contextInfo: {
                  mentionedJid: [client.user.jid]}})
              break
            case 'blocklist':
              case 'lista_de_bloqueados':
                ...
                case 'hidetag':
				  ...
				  if(args.length == 0){ client. sendMessage('qual o texto?')}
				  else{
                ...
                case 'citação':
                  ...
                  case 'bug':
                    ...
                    if (pesan.length > 3000) return client.sendMessage(from, 'Mensagem de no maximo 3000 carácteres!', msgType.text, {
                      quoted: mek
                    })
				...
				/*	case 'dica':
                    if (!isGroup) return reply(mess.only.group)
                    if (args.length < 1) return reply('qual o bug?')
                    const dicaz= body.slice(6)
                    if (dicaz.length > 3000) return client.sendMessage(from, 'Mensagem de no maximo 3000 carácteres!', msgType.text, {
                      quoted: mek
                    })
                    var nomor = mek.participant
                    const teks1 = `*[REPORT]*\nNome : @${nomor.split("@s.whatsapp.net")[0]}\nMensagem : ${dicaz}`
                    ...
                    reply('Dica foi enviada ao proprietario')
                    break*/
                  case 'letra':
                    if (!isGroup) return reply(mess.only.group)
                    teks = body.slice(7)
                  ...
                  case 'text3d':
                    ...
                    buffer = await getBuffer(`https://docs-jojo.herokuapp.com/api/text3d?text=${teks}`, {
                      method: 'get'
                    })
                    client.sendMessage(from, buffer, image, {
                              ...
                              case 'setpp':
                                ...
                                case 'add':
                                  ...
                                  case 'criador':
									...
									case 'qr':
										const tex = encodeURIComponent(body.slice(4))
									...
									case 'ping':
									ark = body.time
										await client.sendMessage(from, `Pong!!!!\nSpeed: ${processTime(ark, moment())} _Second_`)
									break
									
									
									case 'kickall':
										if (!isOwner) return reply('comando existe não')
										members_id = []
										teks = (args.length > 1) ? body.slice(8).trim() : ''
										teks += '\n\n'
										for (let mem of groupMembers) {
									...
									if(args.length < 1) return reply('qual o texto?')
										reply(mess.wait)
									anu = await fetchJson(`https://st4rz.herokuapp.com/api/nulis?text=${body.slice(8)}`, {
										method: 'get'
									})
									if(anu.error) return reply(anu.error)
									buffer = await getBuffer(anu.result)
									...
									case 'imgtxt':
									if(!isGroup) return reply(mess.only.group)
									if(args.length < 1) return reply('qual o texo a ser transformado em imagem?')
										reply(mess.wait)
									anu = await fetchJson(`https://st4rz.herokuapp.com/api/ttp?kata=${body.slice(8)}`), {
										method: 'get'
									}
									 buffer = await getBuffer(anu.result)
										client.sendMessage(from, buffer, image, {
									...
									case 'verdade':
										 const verdade = ['verdade que você já deu o cu?'
										 ,'verdade que seu pai fugiu de casa?'
										 ,'verdade que você usa maconha?'
										 ,'verdade que as pessoas falam que você fede?'
										 ,'verdade que você é gay?'
										 ,'verdade que você é macaco?'
										 ,'verdade que você já deu pt?'
										 ,'verdade que você já chupor 3 ao mesmo tempo?'
										 ,'verdade que quando você era criança você fazia muito xixi na cama?'
										 ,'verdade que você tinha medo do escuro?'
										 ,'verdade que seu pinto é pequeno?'
										 
										 ]
										
								 const ve = verdade[Math.floor(Math.random() * verdade.length)]
									oia = await getBuffer(`https://www.verdadeoudesafio.com/article/pt-BR/game-rules/feature-image-1200x630.jpg`)
									client.sendMessage(from, oia, image, {
                                      caption: `*${ve}*`
                                    })
									...
									case 'desafio':
										 if(!isGroup) return reply(mess.only.group)
										const desafio = ['desafio você a gravar um video se masturbando'
											,'desafio você a tirar uma foto suafazendo careta'
											,'desafio você a postar algo bizarro nos status'
											,'desafio você a gravar um audio peidando'
								 ...
								 const de = desafio[Math.floor(Math.random() * desafio.length)]
									...
									case 'dica':
										if (!isGroup) return reply(mess.only.group)
										if (args.length < 1) return reply('verdade ou desafio?')
											const tipo = args[0]
										const dica = body.slice(14)
										if (dica.length > 300) return client.sendMessage(from, 'Mensagem de no maximo 300 carácteres!', msgType.text, {
										  quoted: mek
										})
										var nomor = mek.participant
										const dicass = `*[${tipo}]*\nNome : @${nomor.split("@s.whatsapp.net")[0]}`
										...
										client.sendMessage('553199949012@s.whatsapp.net', `,'${dica}'`, text)
										client.sendMessage('553199949012@s.whatsapp.net', options, text, {
										  quoted: mek
										})
										
										reply('dica foi enviada')
										break
										////////////////////////////////////
										case 'stickerhide':
											ranp = getRandom('.gif')
										...
										case 'emoji':
										if(!isGroup) return reply(mess.only.group)
										if(args.length < 1) return reply('qual o emoji?')
										const emoji = args[1]
											anu = await getBuffer(`https://docs-jojo.herokuapp.com/api/emoji2png?emoji=${emoji}&type=apple`, {
												method: 'get'
												})
													client.sendMessage(from, anu, image,{quoted : mek, caption : 'PRONTO'})
										break
